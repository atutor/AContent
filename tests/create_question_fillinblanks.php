...
if (isset($_POST['cancel']) || isset($_POST['submit_no'])) {
	...
	if (!$msg->containsErrors()) {
			...
			foreach ($words_in_text as $word) {
				$transform_text .= '<span style="display: inline; border-style: solid; border-width:1px; border-color: 	#C0C0C0; padding-left: 3px; padding-right: 3px; margin-left: 2px; margin-right: 2px;">';
				$transform_text .= '<input type="checkbox" id="'.$word.'_'.$count.'" name="word[]" value="'.$word.'_'.$count.'" />';
	...
	$_POST['answer'] = inval($_POST['answer']);
