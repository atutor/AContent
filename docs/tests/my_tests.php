...
while ($row = mysql_fetch_assoc($result)) {
	...
	if ( ($row['us'] <= time() && $row['ue'] >= time()) && 
	   ( ($row['num_takes'] == TR_TESTS_TAKE_UNLIMITED) || ($takes['cnt'] < $row['num_takes']) )  ) {
		echo '<strong><a href="'.url_rewrite('tests/test_intro.php?tid='.$row['test_id']).'">'.AT_print($row['title'], 'tests.title').'</a></strong>';
	} else {
	...
	echo '<td>'.TR_date($startend_date_long_format, $row['start_date']).'</td>';
	echo '<td>'.TR_date($startend_date_long_format, $row['end_date']).'</td>';

	if ($row['num_takes'] == TR_TESTS_TAKE_UNLIMITED) {
...
if ($row = mysql_fetch_assoc($result)) {
	...
	do {
		...
		echo '<td>'.TR_date($startend_date_long_format, $row['date_taken']).'</td>';
		...
		if ($row['out_of'] == 0) {
			...
			echo _AT('unmarked');
		} elseif (($row['final_score'] != '') && ($row['result_release']==TR_RELEASE_NEVER)) {
			echo _AT('unreleased');
		} else {
			if ($row['random']) {
		...
		if ( ($row['result_release']==TR_RELEASE_IMMEDIATE) || (($row['final_score'] != '') && ($row['result_release']==TR_RELEASE_MARKED)) ) {
			echo '<a href="tests/view_results.php?tid='.$row['test_id'].SEP.'rid='.$row['result_id'].'">'._AT('view_results').'</a>';
		} else {
