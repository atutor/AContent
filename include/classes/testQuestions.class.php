 ...
 *
 *     /themes/default/tests/test_questions/{PREFIX}.tmpl.php
 *     /themes/default/tests/test_questions/{PREFIX}_qti_2p1.tmpl.php
 *     /themes/default/tests/test_questions/{PREFIX}_result.tmpl.php
 *     /themes/default/tests/test_questions/{PREFIX}_stats.tmpl.php
 *
...
/*abstract */ class AbstractTestQuestion  {
	...
	*/
	/*final public */function displayResult($row, $answer_row, $editable = FALSE) {
		...
		$this->savant->display('tests/test_questions/' . $this->sPrefix . '_result.tmpl.php');
	...
	*/
	/*final public */function displayResultStatistics($row, $answers) {
		...
		$this->savant->display('tests/test_questions/' . $this->sPrefix . '_stats.tmpl.php');
	}
	...
	/*final public */function exportQTI($row, $encoding, $version) {
		...
		if ($version=='2.1') {
			$xml = $this->savant->fetch('tests/test_questions/'. $this->sPrefix . '_qti_2p1.tmpl.php');
		} else {	
			$xml = $this->savant->fetch('tests/test_questions/'. $this->sPrefix . '_qti_1p2.tmpl.php');
		}
		return $xml;
	}
	...
	*/
	/*final private */function displayHeader($weight, $score = FALSE, $question_id = FALSE) {
		...
		$this->savant->display('tests/test_questions/header.tmpl.php');
	}
	...
	*/
	/*final private */function displayFooter() {
		$this->savant->display('tests/test_questions/footer.tmpl.php');
	}
	...
	function importQTI($question){
		...
		if (!$msg->containsErrors()) {
			...
			for ($i=0; $i<10; $i++) {
				 ...
				 * @harris
				 */
				$question['choice'][$i] = Utility::validateLength($question['choice'][$i], 255);
